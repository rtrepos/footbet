...
def get_model():
    if mu.mutex_process('data/model2.pkl'):
        model_y = tfk.Sequential([
            tfk.layers.InputLayer(batch_input_shape=(None, 20, 8+15), name='in'),
            tfk.layers.Conv1D(filters=80, kernel_size=2, strides=1, name='conv1'),
            tfk.layers.MaxPooling1D(pool_size=4, name='pool1'),
            tfk.layers.Dropout(.2),
            tfk.layers.Conv1D(filters=20, kernel_size=3, strides=1, name='conv2'),
            tfk.layers.MaxPooling1D(pool_size=2, name='pool2'),
            tfk.layers.Dropout(.2),
            tfk.layers.Flatten(name='flat'),
            # tfk.layers.Dense(20, activation=tf.nn.softmax, name='dense1'),
...
def model_predict():
    ...
    home_teams = ["Brest", "Lyon", "Metz", "Nice", "St Etienne", "Bordeaux", "Lille", "Montpellier", "Nantes", "Strasbourg"]
    away_teams = ["Dijon", "Rennes", "Angers", "Nimes", "Lens", "Paris SG", "Marseille", "Lorient", "Reims", "Monaco"]
    x_predict = np.zeros(shape=(1, 20, 8+15))
